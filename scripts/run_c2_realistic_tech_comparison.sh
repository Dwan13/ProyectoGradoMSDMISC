...
OUT_DIR="${ROOT_DIR}/Testing/results/c2-mesh-realistic-${STAMP}"
QOS_CSV="${OUT_DIR}/c2-realistic-qos.csv"
FAULT_CSV="${OUT_DIR}/c2-realistic-fault.csv"
RESOURCE_BY_TECH_CSV="${OUT_DIR}/c2-realistic-resource-by-tech.csv"
SUMMARY_MD="${OUT_DIR}/c2-realistic-summary.md"
...
ensure_linkerd() {
  if ! microk8s kubectl get ns linkerd >/dev/null 2>&1; then
    echo "[ERROR] C2 requiere Linkerd (namespace 'linkerd' no encontrado)"
    exit 2
  fi
}

ensure_istio() {
  if ! microk8s kubectl get ns istio-system >/dev/null 2>&1; then
    echo "[ERROR] C2 requiere Istio (namespace 'istio-system' no encontrado)"
    exit 2
  fi
}

rollout_realistic() {
  microk8s kubectl rollout restart deployment/auth-service -n realistic >/dev/null
  microk8s kubectl rollout restart deployment/api-service -n realistic >/dev/null
  microk8s kubectl rollout restart deployment/data-service -n realistic >/dev/null
  microk8s kubectl rollout status deployment/auth-service -n realistic --timeout=240s >/dev/null
  microk8s kubectl rollout status deployment/api-service -n realistic --timeout=240s >/dev/null
  microk8s kubectl rollout status deployment/data-service -n realistic --timeout=240s >/dev/null
}

apply_mesh_none() {
  microk8s kubectl label namespace realistic linkerd.io/inject- --overwrite >/dev/null 2>&1 || true
  microk8s kubectl label namespace realistic istio-injection- --overwrite >/dev/null 2>&1 || true
  microk8s kubectl label namespace realistic istio.io/rev- --overwrite >/dev/null 2>&1 || true
  rollout_realistic
}
...
apply_mesh_linkerd() {
  microk8s kubectl label namespace realistic istio-injection- --overwrite >/dev/null 2>&1 || true
  microk8s kubectl label namespace realistic istio.io/rev- --overwrite >/dev/null 2>&1 || true
  microk8s kubectl label namespace realistic linkerd.io/inject=enabled --overwrite >/dev/null
  rollout_realistic
}
...
apply_mesh_istio() {
  microk8s kubectl label namespace realistic linkerd.io/inject- --overwrite >/dev/null 2>&1 || true
  microk8s kubectl label namespace realistic istio.io/rev- --overwrite >/dev/null 2>&1 || true
  microk8s kubectl label namespace realistic istio-injection=enabled --overwrite >/dev/null
  rollout_realistic
}
...
ensure_prometheus() {
  ...
  microk8s kubectl port-forward -n observability svc/kube-prom-stack-kube-prome-prometheus 9090:9090 >/tmp/c2r-prom-pf.log 2>&1 &
  PF_PROM_PID=$!
...
capture_resource_snapshot() {
  ...
  if [[ -n "${mesh_ns}" ]]; then
    mesh_cpu="$(query_prom_scalar "sum(rate(container_cpu_usage_seconds_total{namespace=\"${mesh_ns}\",container!=\"\",pod!=\"\"}[2m]))")"
    mesh_mem_bytes="$(query_prom_scalar "sum(container_memory_working_set_bytes{namespace=\"${mesh_ns}\",container!=\"\",pod!=\"\"})")"
  fi

  python3 - "${RESOURCE_BY_TECH_CSV}" "${tech}" "${realistic_cpu}" "${realistic_mem_bytes}" "${mesh_cpu}" "${mesh_mem_bytes}" <<'PY'
import csv
import os
import sys

csv_path, tech, real_cpu, real_mem_b, mesh_cpu, mesh_mem_b = sys.argv[1:7]
real_cpu = float(real_cpu or 0)
real_mem_b = float(real_mem_b or 0)
mesh_cpu = float(mesh_cpu or 0)
mesh_mem_b = float(mesh_mem_b or 0)

real_mem_mib = real_mem_b / (1024 * 1024)
mesh_mem_mib = mesh_mem_b / (1024 * 1024)
e2e_cpu = real_cpu + mesh_cpu
e2e_mem_mib = real_mem_mib + mesh_mem_mib
...
start_baseline_pf() {
  ...
  microk8s kubectl port-forward -n realistic svc/auth-service 18082:8080 >/tmp/c2r-auth-pf.log 2>&1 &
  PF_AUTH_PID=$!
  microk8s kubectl port-forward -n realistic svc/api-service 18081:8080 >/tmp/c2r-api-pf.log 2>&1 &
  PF_API_PID=$!
...
run_users_bulk_case() {
  ...
  for attempt in $(seq 1 "${attempts}"); do
    ...
    if k6 run --no-thresholds \
      ...
      --out json="${raw_json}" \
      "${K6_SCRIPT}" >/tmp/c2r-${tech}.log 2>&1; then
      return 0
    fi
...
run_fault_case() {
  ...
  k6 run --no-thresholds \
    ...
    -e LIST_LIMIT="${LIST_LIMIT}" \
    --out json="${raw_json}" \
    "${K6_SCRIPT}" >/tmp/c2r-fault-${tech}.log 2>&1 &
  local k6_pid=$!
...
for fn in os.listdir(out_dir):
    ...
    def get_metric_value(metric_name, key, default=0.0):
      ...
      # Newer k6 summary-export schema stores values directly at metric root.
      if isinstance(m, dict) and key in m and isinstance(m.get(key), (int, float)):
        return float(m.get(key))
      # Older schema uses a nested "values" object.
...
with open(md_path, 'w', encoding='utf-8') as f:
    f.write('# C2 Realistic Technology Comparison\n\n')
    ...
    f.write('| Tech | Realistic CPU | Mesh CPU | E2E CPU | Realistic Mem (MiB) | Mesh Mem (MiB) | E2E Mem (MiB) |\n')
    f.write('|---|---:|---:|---:|---:|---:|---:|\n')
    for r in resource:
        f.write('| {} | {:.4f} | {:.4f} | {:.4f} | {:.2f} | {:.2f} | {:.2f} |\n'.format(
            r['technology'],
            float(r['realistic_cpu_cores'] or 0),
            float(r['mesh_cpu_cores'] or 0),
            float(r['e2e_cpu_cores'] or 0),
            float(r['realistic_mem_mib'] or 0),
            float(r['mesh_mem_mib'] or 0),
...
main() {
  ...
  log "[2/9] C2 mTLS (Istio) realistic"
  apply_mesh_istio
  start_baseline_pf
  run_users_bulk_case istio-mtls-realistic "http://127.0.0.1:18082" "http://127.0.0.1:18081"
  capture_resource_snapshot istio-mtls-realistic "istio-system"

  log "[3/9] C2 mTLS (Linkerd) realistic"
  apply_mesh_linkerd
  start_baseline_pf
  run_users_bulk_case linkerd-mtls-realistic "http://127.0.0.1:18082" "http://127.0.0.1:18081"
  capture_resource_snapshot linkerd-mtls-realistic "linkerd"
  ...
  log "[6/9] Fault C2 mTLS (Istio)"
  apply_mesh_istio
  start_baseline_pf
  run_fault_case istio-mtls-realistic "http://127.0.0.1:18082" "http://127.0.0.1:18081" "http://127.0.0.1:18081/health"

  log "[7/9] Fault C2 mTLS (Linkerd)"
  apply_mesh_linkerd
  start_baseline_pf
  run_fault_case linkerd-mtls-realistic "http://127.0.0.1:18082" "http://127.0.0.1:18081" "http://127.0.0.1:18081/health"
  ...
  if [[ "${AUTO_CLEANUP_RESULTS}" == "1" ]] && [[ -x "${ROOT_DIR}/scripts/cleanup_results.sh" ]]; then
    ...
    if [[ "${KEEP_RAW_JSON}" == "1" ]]; then
      KEEP_RUNS="${KEEP_RUNS}" DELETE_RAW_IN_KEPT="0" "${ROOT_DIR}/scripts/cleanup_results.sh" --keep-runs "${KEEP_RUNS}" >/tmp/c2r-cleanup.log 2>&1 || true
    else
      KEEP_RUNS="${KEEP_RUNS}" DELETE_RAW_IN_KEPT="1" "${ROOT_DIR}/scripts/cleanup_results.sh" --keep-runs "${KEEP_RUNS}" >/tmp/c2r-cleanup.log 2>&1 || true
    fi
  fi

  log "[9/9] Limpiando mesh labels"
  apply_mesh_none
