...
C3_BASIC_YAML="${REAL_DIR}/k8s/08-c3-networkpolicy.yaml"
C3_STRICT_YAML="${REAL_DIR}/k8s/08-c3-networkpolicy-strict.yaml"
STAMP="$(date +%Y%m%d_%H%M%S)"
OUT_DIR="${ROOT_DIR}/Testing/results/c3-realistic-${STAMP}"
QOS_CSV="${OUT_DIR}/c3-realistic-qos.csv"
FAULT_CSV="${OUT_DIR}/c3-realistic-fault.csv"
RESOURCE_BY_TECH_CSV="${OUT_DIR}/c3-realistic-resource-by-tech.csv"
SUMMARY_MD="${OUT_DIR}/c3-realistic-summary.md"
...
apply_c3_none() {
  microk8s kubectl delete -f "${C3_BASIC_YAML}" --ignore-not-found >/dev/null 2>&1 || true
  microk8s kubectl delete -f "${C3_STRICT_YAML}" --ignore-not-found >/dev/null 2>&1 || true
  rollout_realistic
}

apply_c3_basic() {
  microk8s kubectl delete -f "${C3_STRICT_YAML}" --ignore-not-found >/dev/null 2>&1 || true
  microk8s kubectl apply -f "${C3_BASIC_YAML}" >/dev/null
  rollout_realistic
}

apply_c3_strict() {
  microk8s kubectl delete -f "${C3_BASIC_YAML}" --ignore-not-found >/dev/null 2>&1 || true
  microk8s kubectl apply -f "${C3_STRICT_YAML}" >/dev/null
  rollout_realistic
}
...
ensure_prometheus() {
  ...
  microk8s kubectl port-forward -n observability svc/kube-prom-stack-kube-prome-prometheus 9090:9090 >/tmp/c3r-prom-pf.log 2>&1 &
  PF_PROM_PID=$!
...
capture_resource_snapshot() {
  ...
  policy_cpu="$(query_prom_scalar 'sum(rate(container_cpu_usage_seconds_total{namespace="kube-system",pod=~"calico-node.*",container!=""}[2m]))')"
  policy_mem_bytes="$(query_prom_scalar 'sum(container_memory_working_set_bytes{namespace="kube-system",pod=~"calico-node.*",container!=""})')"

  python3 - "${RESOURCE_BY_TECH_CSV}" "${tech}" "${realistic_cpu}" "${realistic_mem_bytes}" "${policy_cpu}" "${policy_mem_bytes}" <<'PY'
import csv
import os
import sys

csv_path, tech, real_cpu, real_mem_b, pol_cpu, pol_mem_b = sys.argv[1:7]
real_cpu = float(real_cpu or 0)
real_mem_b = float(real_mem_b or 0)
pol_cpu = float(pol_cpu or 0)
pol_mem_b = float(pol_mem_b or 0)

real_mem_mib = real_mem_b / (1024 * 1024)
pol_mem_mib = pol_mem_b / (1024 * 1024)
e2e_cpu = real_cpu + pol_cpu
e2e_mem_mib = real_mem_mib + pol_mem_mib
...
start_baseline_pf() {
  ...
  microk8s kubectl port-forward -n realistic svc/auth-service 18082:8080 >/tmp/c3r-auth-pf.log 2>&1 &
  PF_AUTH_PID=$!
  microk8s kubectl port-forward -n realistic svc/api-service 18081:8080 >/tmp/c3r-api-pf.log 2>&1 &
  PF_API_PID=$!
...
run_users_bulk_case() {
  ...
  for attempt in $(seq 1 "${attempts}"); do
    ...
    if k6 run --no-thresholds \
      ...
      --out json="${raw_json}" \
      "${K6_SCRIPT}" >/tmp/c3r-${tech}.log 2>&1; then
      return 0
    fi
...
run_fault_case() {
  ...
  k6 run --no-thresholds \
    ...
    -e LIST_LIMIT="${LIST_LIMIT}" \
    --out json="${raw_json}" \
    "${K6_SCRIPT}" >/tmp/c3r-fault-${tech}.log 2>&1 &
  local k6_pid=$!
...
with open(md_path, 'w', encoding='utf-8') as f:
    f.write('# C3 Realistic Technology Comparison\n\n')
    ...
    f.write('| Tech | Realistic CPU | Policy CPU | E2E CPU | Realistic Mem (MiB) | Policy Mem (MiB) | E2E Mem (MiB) |\n')
    f.write('|---|---:|---:|---:|---:|---:|---:|\n')
    for r in resource:
        f.write('| {} | {:.4f} | {:.4f} | {:.4f} | {:.2f} | {:.2f} | {:.2f} |\n'.format(
            r['technology'],
            float(r['realistic_cpu_cores'] or 0),
            float(r['policy_cpu_cores'] or 0),
            float(r['e2e_cpu_cores'] or 0),
            float(r['realistic_mem_mib'] or 0),
            float(r['policy_mem_mib'] or 0),
...
main() {
  ...
  log "[2/9] C3 netpol basic"
  apply_c3_basic
  start_baseline_pf
  run_users_bulk_case c3-netpol-basic-realistic "http://127.0.0.1:18082" "http://127.0.0.1:18081"
  capture_resource_snapshot c3-netpol-basic-realistic

  log "[3/9] C3 netpol strict"
  apply_c3_strict
  start_baseline_pf
  run_users_bulk_case c3-netpol-strict-realistic "http://127.0.0.1:18082" "http://127.0.0.1:18081"
  capture_resource_snapshot c3-netpol-strict-realistic
  ...
  log "[6/9] Fault C3 netpol basic"
  apply_c3_basic
  start_baseline_pf
  run_fault_case c3-netpol-basic-realistic "http://127.0.0.1:18082" "http://127.0.0.1:18081" "http://127.0.0.1:18081/health"

  log "[7/9] Fault C3 netpol strict"
  apply_c3_strict
  start_baseline_pf
  run_fault_case c3-netpol-strict-realistic "http://127.0.0.1:18082" "http://127.0.0.1:18081" "http://127.0.0.1:18081/health"
  ...
  if [[ "${AUTO_CLEANUP_RESULTS}" == "1" ]] && [[ -x "${ROOT_DIR}/scripts/cleanup_results.sh" ]]; then
    ...
    if [[ "${KEEP_RAW_JSON}" == "1" ]]; then
      KEEP_RUNS="${KEEP_RUNS}" DELETE_RAW_IN_KEPT="0" "${ROOT_DIR}/scripts/cleanup_results.sh" --keep-runs "${KEEP_RUNS}" >/tmp/c3r-cleanup.log 2>&1 || true
    else
      KEEP_RUNS="${KEEP_RUNS}" DELETE_RAW_IN_KEPT="1" "${ROOT_DIR}/scripts/cleanup_results.sh" --keep-runs "${KEEP_RUNS}" >/tmp/c3r-cleanup.log 2>&1 || true
    fi
  fi

  log "[9/9] Limpiando C3 (baseline)"
  apply_c3_none
