...
# Quitar policies para baseline
bash "$ROOT_DIR/policies/remove-policies.sh"
...
ensure_portforward() {
  ...
  echo "[baseline-netpol] Warning: port-forward no estable"
  return 0
}
...
for vus in 5 10 20; do
  for rep in 1 2 3; do
    ...
    logfile="$LOG_DIR/baseline-netpol-vus${vus}-rep${rep}.log"

    ensure_portforward
    echo "[baseline-netpol] Ejecutando VUS=${vus} REP=${rep}"
    k6 run --no-thresholds \
      -e TARGET_URL="$TARGET_URL" \
      -e VUS="$vus" \
      -e DURATION="60s" \
      --out "json=$outfile" \
      "$ROOT_DIR/tests/test-netpol-baseline.js" \
      >"$logfile" 2>&1 || true
...
echo "[baseline-netpol] Finalizado"
